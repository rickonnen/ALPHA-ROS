...
export function DatosAvisoForm({ onNext, onBack, submitRef }: DatosAvisoFormProps) {
  ...
  const inputBase = 'w-full border rounded-md px-3 py-2 text-base outline-none bg-background text-foreground placeholder:text-muted-foreground transition-colors'
  const inputClass = (hasError: boolean) =>
    `${inputBase} ${hasError ? 'border-destructive' : 'border-border focus:border-primary'}`

  return (
    <div className="flex flex-col gap-5 h-full pt-3">

      {/* Título del aviso */}
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="titulo" className="text-base font-medium text-foreground">
        ...
        <input
          ...
          className={inputClass(!!(touched.titulo && errors.titulo))}
        />
      ...
      <div className="flex flex-col gap-1.5" ref={dropdownRef}>
        <Label htmlFor="tipoOperacion" className="text-base font-medium text-foreground">
        ...
        <div className="relative">
          <button
            ...
            className={`w-full h-[40px] px-3 text-base bg-background rounded-md border outline-none flex items-center justify-between transition-colors
              ${touched.tipoOperacion && errors.tipoOperacion ? 'border-destructive' : dropdownOpen ? 'border-primary' : 'border-border'}
          ...
          {dropdownOpen && (
            <ul
              ...
              role="listbox"
              className="absolute top-[calc(100%+4px)] left-0 right-0 z-50 bg-background border border-border rounded-md shadow-md py-1 max-h-60 overflow-auto"
            >
              {TIPOS_OPERACION.map((opcion: string) => (
                <li
                  key={opcion}
                  role="option"
                  aria-selected={opcion === values.tipoOperacion}
      ...
      <div className="flex flex-row items-start gap-3">
        ...
        <div className="flex flex-col gap-1.5 flex-1 min-w-0">
          <Label htmlFor="precio" className="text-base font-medium text-foreground">
            Precio
          </Label>
          <input
            id="precio"
            ...
            placeholder="0.00"
            className={inputClass(!!(touched.precio && errors.precio))}
          />
          <span className="text-destructive text-xs h-4 block">
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          <Label className="text-base font-medium text-foreground">Tipo de moneda</Label>
          <div
            className="relative flex items-center rounded-md bg-background border border-border"
            style={{ height: '38px', width: '110px' }}
          >
            {/* Píldora deslizante */}
            <div
              className="absolute top-0 bottom-0 w-1/2 rounded-md bg-secondary"
              style={{
            ...
            <button
              ...
              className={`relative z-10 flex-1 h-full border-none bg-transparent text-sm font-semibold cursor-pointer transition-colors
                ${values.tipoMoneda === 'USD' ? 'text-secondary-foreground' : 'text-foreground'}
              `}
            >
              USD
            </button>
            <button
              type="button"
              onClick={() => handleMoneda('Bs')}
              className={`relative z-10 flex-1 h-full border-none bg-transparent text-sm font-semibold cursor-pointer transition-colors
                ${values.tipoMoneda === 'Bs' ? 'text-secondary-foreground' : 'text-foreground'}
              `}
            >
              Bs
            </button>
