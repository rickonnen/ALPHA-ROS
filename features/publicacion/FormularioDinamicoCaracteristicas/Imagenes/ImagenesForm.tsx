 ...
 * @param {ImagenesFormProps} props - onNext, onBack, submitRef, onImagesChange
...
interface ImagenesFormProps {
  ...
  onImagesChange:  (files: File[]) => void   // ← prop añadido
}

export function ImagenesForm({ onNext, onBack, submitRef, onImagesChange }: ImagenesFormProps) {
  const {
    values,
    errors,
    touched,
  ...
  // ── Registrar el trigger de validación en submitRef ──
  useEffect(() => {
    submitRef.current = () => handleSubmit(() => onNext())
    return () => { submitRef.current = null }
  }, [handleSubmit, onNext, submitRef])

  // ── Notificar a la página cada vez que cambian las imágenes ──
  useEffect(() => {
