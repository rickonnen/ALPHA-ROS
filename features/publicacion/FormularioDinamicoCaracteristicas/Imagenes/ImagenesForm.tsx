...
export function ImagenesForm({
  ...
  useEffect(() => { onImagesChange(values.imagenes) }, [values.imagenes, onImagesChange])
  useEffect(() => { onUrlsChange?.(urlsExistentes, urlsABorrar) }, [urlsExistentes, urlsABorrar, onUrlsChange])
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      {!hasFiles && (
        <div
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed cursor-pointer transition-colors min-h-[220px] ${
            dragging ? 'border-secondary bg-secondary/10' : 'border-border bg-muted'
          }`}
        >
          ...
          <div className="text-center">
            <p className="text-base font-semibold text-primary">Arrastra tus imagenes aqui</p>
          ...
          <button
            ...
            className="mt-1 px-5 py-2 text-base font-semibold text-primary-foreground bg-primary rounded-full hover:opacity-90 transition-opacity"
          >
      ...
      {hasFiles && (
        <div className="flex flex-col gap-3 flex-1">
          ...
          {/* Imagen principal */}
          <div className="relative rounded-xl overflow-hidden flex-1 bg-background flex items-center justify-center min-h-[180px]">
            <img
            ...
            <button
              ...
              onClick={() => {
                ...
                if (esUrlExistente) {
                  ...
                  handleEliminar(selectedIdx - urlsExistentes.length)
                }
          ...
          <div className="flex gap-2">
            {previewsCombinados.map((src, idx) => (
              <button
                ...
                className={`relative rounded-lg overflow-hidden transition-all bg-muted flex-1 min-w-0 max-w-[100px] h-[70px] border-2 ${
                  idx === selectedIdx ? 'border-secondary' : 'border-transparent'
                }`}
              >
                <img
                  src={src}
            ...
            {puedeAgregar && (
              <button
                type="button"
                onClick={() => inputRef.current?.click()}
                className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-muted gap-0.5 transition-colors hover:border-secondary flex-1 min-w-0 max-w-[100px] h-[70px]"
              >
