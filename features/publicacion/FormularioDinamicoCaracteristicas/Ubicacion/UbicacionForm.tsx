    ...
    <>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-[#1A1714]">Selecciona la ubicación</h3>
              <button type="button" onClick={() => setMapaAbierto(false)}
                className="text-gray-400 hover:text-gray-600 text-xl leading-none"
              >✕</button>
            </div>
            <LocationPicker onChange={(data) => { handleUbicacion(data); setMapaAbierto(false) }} />
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-5 h-full" style={{ paddingTop: '12px' }}>
        ...
        <div className="flex flex-col gap-1.5" ref={dropdownRef}>
          ...
          <button
            type="button" role="combobox" aria-haspopup="listbox" aria-expanded={dropdownOpen}
          ...
          >
            ...
            <svg className={`w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0 ${dropdownOpen ? 'rotate-180' : ''}`}
            ...
            <div className="relative z-50">
              ...
              >
                {DEPARTAMENTOS.map((depto: string) => (
                  <li key={depto} role="option" aria-selected={depto === values.departamento}
        ...
        {/* Zona */}
        <div className="flex flex-col gap-1.5">
          <Label className="text-sm font-medium text-[#1A1714]">Especifique Zona</Label>
          <input
            type="text" value={values.zona} maxLength={MAX_ZONA}
