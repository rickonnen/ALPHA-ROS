...
export function usePublicar({
  ...
  const handlePublicar = useCallback(async () => {
    ...
    try {
      const datosAviso = JSON.parse(
        sessionStorage.getItem("datosAviso") ?? "{}",
      );
      const categoria = JSON.parse(
        sessionStorage.getItem("categoriaYEstado") ?? "{}",
      );
      const ubicacion = JSON.parse(sessionStorage.getItem("ubicacion") ?? "{}");
      const caracteristicas = JSON.parse(
        sessionStorage.getItem("caracteristicasDetalle") ?? "{}",
      );
      const video = JSON.parse(
        sessionStorage.getItem("videoPropiedad") ?? "{}",
      );
      ...
      formData.append(
        "estadoConstruccion",
        String(ESTADO_IDS[categoria.estadoPropiedad as string] ?? 1),
      );
      ...
      formData.append(
        "habitaciones",
        toNullableStr(caracteristicas.habitaciones),
      );
      ...
      formData.append(
        ...
        JSON.stringify(caracteristicasExtras),
      );
      ...
      if (modoEdicion && idPublicacion) {
        ...
        urlsQueQuedanRef.current.forEach((url) =>
          formData.append("imagenesViejas", url),
        );
        urlsABorrarRef.current.forEach((url) =>
          formData.append("imagenesABorrar", url),
        );
        ...
        if (result.success) {
          ...
          const firstError = Object.values(result.errors ?? {}).flat()[0] as
            | string
            | undefined;
        ...
        if (result.success) {
          //HU-6: Envio de notificaciones por whatsapp 
          try {
            await fetch("/api/whatsapp/publicacion", {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                userId: USER_ID ?? null,
                idPublicacion: result.idPublicacion,
                titulo: datosAviso.titulo,
                tipoOperacion: datosAviso.tipoOperacion,
                tipoInmueble: categoria.tipoPropiedad,
                precio: datosAviso.precio,
                tipoMoneda: datosAviso.tipoMoneda,
            ...
            console.error("No se pudo enviar WhatsApp de publicación:", error);
          }
          SESSION_KEYS_TO_CLEAN.forEach((k) => {
            try {
              sessionStorage.removeItem(k);
      ...
      setPublishError(
        `Error inesperado: ${err instanceof Error ? err.message : JSON.stringify(err)}`,
      );
    } finally {
      setIsPublishing(false);
    }
  ...
  const handleSidebarClick = useCallback(
    (index: number) => {
      ...
      const intermediosCompletos = STEPS.slice(0, index).every(
        (step, i) =>
          i === currentStep || step.opcional || completedSteps.has(i),
      );
      if (!intermediosCompletos) {
        setBlockMsg(
          "Debes completar los pasos anteriores antes de avanzar a este.",
        );
        return;
      }
      if (currentStep === 5) {
