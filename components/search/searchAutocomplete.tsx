  ...
  return (
    <div ref={wrapperRef} className="relative w-72">
      <input
        ...
        className="w-full rounded-md border border-[#E7E1D7] bg-[#E7E1D7] px-3 py-2 text-sm text-[#2E2E2E] outline-none"
      />

      {abierto && (
        <ul className="absolute top-full z-50 mt-1 w-full rounded-md border border-[#E7E1D7] bg-white shadow-sm">
          {cargando && (
            <li className="px-3 py-2 text-sm text-gray-500">Buscando ubicaciones...</li>
          )}
          {error && !cargando && (
          ...
          {!error &&
            ...
            sugerencias.map((feature) => (
              <li
                ...
                className="cursor-pointer px-3 py-2 text-sm text-[#2E2E2E] hover:bg-[#E7E1D7]"
              >
                {feature.place_name}
              </li>
            ))}
