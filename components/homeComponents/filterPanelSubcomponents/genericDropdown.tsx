...
export default function GenericDropdown({
  ...
  // clases base y estilos dinámicos extraídos en variables para mayor legibilidad
  const strDropdownBaseClass = "w-full flex items-center justify-between gap-2 px-4 py-2 rounded-full text-sm font-medium border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-secondary-fund";
  ...
  return (
    <div className="relative flex-1 md:flex-none">
      ...
      {bolIsOpen && (
        <div role="listbox" className="absolute top-[calc(100%+8px)] left-0 z-50 w-full md:w-52 bg-background rounded-xl border border-border shadow-lg overflow-hidden p-1">
          {arrOptions.map((strOption, index) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                  bolIsSelected || bolIsHovered ? "bg-secondary-fund text-foreground font-bold" : "text-foreground hover:bg-secondary-fund/60"
                } ${optionHover}`}
              >
                <span>{strOption}</span>
                {bolIsMultiple && (
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${bolIsSelected ? "bg-primary border-primary" : "border-muted-foreground bg-background"}`}>
                    {bolIsSelected && <div className="w-1.5 h-1.5 bg-white rounded-full" /> }
                  </div>
                )}
              </button>
            );
