      ...
      <div className="max-w-5xl mx-auto">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsList className="hidden sm:flex justify-center bg-transparent h-auto p-0 gap-8 mb-12">
            <TabsTrigger value="publicacion" className="px-10 py-3.5 rounded-full bg-muted text-muted-foreground font-bold text-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all shadow-sm hover:bg-muted/80">
              Planes de publicación
            </TabsTrigger>
            <TabsTrigger value="promocion" className="px-10 py-3.5 rounded-full bg-muted text-muted-foreground font-bold text-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all shadow-sm hover:bg-muted/80">
              Planes de promoción
            </TabsTrigger>
          </TabsList>
          ...
          {!showForm && (
            <div className="flex justify-end mb-8">
              <Button
                onClick={() => { setPlanToEdit(null); setShowForm(true); }}
          ...
          <TabsContent value={activeTab} className="m-0 outline-none">
            {showForm && (
              <PlanForm 
                ...
                onCancel={() => { setShowForm(false); setPlanToEdit(null); }} 
