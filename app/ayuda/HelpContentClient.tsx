...
const helpContent: Record<string, { subtitle: string; description: string; proTip: string; steps: string[]; images: string[] }> = {
  ...
  publicar: {
    subtitle: "Guía paso a paso para completar correctamente una publicación inmobiliaria.",
    description: "Aprende a registrar tu inmueble de forma clara, completa y atractiva, siguiendo el flujo real del formulario de publicación dentro de PROPBOL.",
    proTip: "Antes de publicar, revisa que tu inmueble tenga al menos 3 fotos claras, un título específico, una descripción útil y la ubicación correctamente definida en el mapa.",
    steps: [
      "Datos del aviso: completa el título, tipo de operación, precio y moneda con información clara y específica.",
      "Categoría y estado: selecciona el tipo de inmueble y el estado correcto: En Planos, En Construcción o Entrega Inmediata, según corresponda.",
      "Ubicación: registra departamento, ciudad, zona y dirección exacta para mejorar la precisión de la búsqueda.",
      "Características: completa habitaciones, baños, superficie y otros atributos principales del inmueble.",
      "Imágenes: sube al menos 3 fotografías con buena iluminación, mostrando los ambientes principales del inmueble.",
      "Video: este paso es opcional; si se agrega, debe respetar los formatos aceptados por la plataforma.",
      "Descripción y características extras: redacta una descripción completa y agrega hasta 4 campos personalizados, por ejemplo Estado de construcción: En construcción o Fecha de entrega: Diciembre 2025."
    ],
    images: [
      "https://placehold.co/1280x720/F4EFE6/1F3A4D?text=Guia+de+publicacion+1",
      "https://placehold.co/1280x720/E7E1D7/1F3A4D?text=Guia+de+publicacion+2",
      "https://placehold.co/1280x720/F4EFE6/C26E5A?text=Guia+de+publicacion+3"
    ]
  },
  pagos: {
...
export default function HelpContentClient() {
  ...
  const objActiveTopic =
    arrHelpTopics.find((objTopic) => objTopic.id === strActiveTopicId) || arrHelpTopics[0];

  const activeContent = helpContent[strActiveTopicId] || helpContent["identificar"];
  const intTotalImages = activeContent.images.length;

  const goToSelectedImage = (intIndex: number) => {
    setIntCurrentIndex(intIndex);
  };

  const goToNextImage = () => {
    setIntCurrentIndex((intPrev) => (intPrev + 1) % intTotalImages);
  };

  const goToPreviousImage = () => {
    setIntCurrentIndex((intPrev) => (intPrev - 1 + intTotalImages) % intTotalImages);
  };
  ...
  useEffect(() => {
    ...
    const intIntervalId = window.setInterval(() => {
      setIntCurrentIndex((intPrev) => (intPrev + 1) % intTotalImages);
    }, 6000);

    return () => window.clearInterval(intIntervalId);
  }, [intTotalImages, strActiveTopicId]);
  ...
  const handleTouchStart = (objEvent: React.TouchEvent<HTMLDivElement>) => {
    setNumTouchStartX(objEvent.touches[0].clientX);
  };

  const handleTouchEnd = (objEvent: React.TouchEvent<HTMLDivElement>) => {
    if (numTouchStartX === null) return;

    const numTouchEndX = objEvent.changedTouches[0].clientX;
    const numDiff = numTouchStartX - numTouchEndX;

    if (Math.abs(numDiff) > 50) {
      if (numDiff > 0) {
        goToNextImage();
      } else {
        goToPreviousImage();
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row gap-8 items-start">
        <aside className="w-full md:w-1/3 lg:w-1/4 md:sticky md:top-24 flex flex-col bg-[#E7E1D7] p-4 rounded-2xl border border-[#C4BAA8] shadow-sm h-fit z-10 shrink-0">
          <h2 className="mb-3 px-2 text-[11px] font-bold uppercase tracking-widest text-[#8b8276] hidden md:block">
            Temas de ayuda
          </h2>
          ...
          <div className="flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0 scrollbar-hide snap-x">
            {arrHelpTopics.map((topic) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 md:py-4 rounded-xl text-left text-sm font-medium transition-all duration-200 whitespace-nowrap md:whitespace-normal shrink-0 snap-start border ${
        ...
        <main
          ...
          className="w-full md:w-2/3 lg:w-3/4 bg-[#E7E1D7] p-6 sm:p-10 rounded-2xl border border-[#C4BAA8] shadow-sm animate-in fade-in duration-500"
        >
          <nav className="flex items-center flex-wrap gap-2 text-xs text-[#8b8276] mb-4">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#1F3A4D] mb-6">
            {objActiveTopic.title}
          </h1>

          <div className="text-[#2E2E2E] text-base sm:text-lg mb-8 leading-relaxed">
            <p className="font-semibold text-[#C26E5A] mb-3">{activeContent.subtitle}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activeContent.steps.map((step, index) => (
                <div
                  key={index}
                  className="flex flex-col gap-3 bg-[#F4EFE6] p-5 rounded-xl border border-[#C4BAA8] transition-all duration-200 hover:border-[#C26E5A]/50"
                >
                  <span className="text-[10px] font-bold text-[#8b8276] uppercase tracking-widest">
                    Paso 0{index + 1}
                  </span>
                  <p className="text-[14px] sm:text-[15px] font-semibold text-[#2E2E2E] leading-snug">
                    {step}
                  </p>
          ...
          <div className="mt-8 border-t border-[#C4BAA8] pt-8">
            <h3 className="text-[10px] font-bold text-[#8b8276] mb-4 uppercase tracking-[3px] text-center">
              Referencia Visual
            </h3>
              ...
              {intTotalImages > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 bg-[#1F3A4D] text-[#E7E1D7] p-2 rounded-full opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all z-10 shadow-md"
                  ...
                  <button
                    ...
                    className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 bg-[#1F3A4D] text-[#E7E1D7] p-2 rounded-full opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all z-10 shadow-md"
            ...
            {activeContent.proTip && (
              <div className="mt-8 sm:mt-10 bg-[#C26E5A] text-[#E7E1D7] p-5 md:p-6 rounded-2xl shadow-sm flex flex-col sm:flex-row items-start gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <h4 className="font-bold text-[13px] sm:text-[14px] uppercase tracking-widest text-[#E7E1D7]/90">
                    Tip Pro
                  </h4>
                  <p className="text-[14px] sm:text-[15px] leading-relaxed font-medium">
                    {activeContent.proTip}
                  </p>
                </div>
              </div>
            ...
            <p className="text-[10px] mt-6 sm:mt-8 text-center text-[#8b8276] uppercase tracking-widest">
