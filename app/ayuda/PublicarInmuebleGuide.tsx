...
const ARR_GALLERY: GalleryImage[] = [
  ...
  {
    url: "https://res.cloudinary.com/dj1mlj3vz/image/upload/v1778437856/parte7.1_llqsql.png",
    ...
    <div className="bg-secondary text-secondary-foreground rounded-[24px] p-6 flex items-start gap-4 w-full">
      ...
      <div>
        ...
        <p className="text-[15px] leading-relaxed font-medium">
          Antes de publicar, revisa que tu inmueble tenga al menos 3 fotos claras, un título específico, una descripción útil y la ubicación correctamente definida.
        </p>
      </div>
    </div>
  );
...
function ActionButton({ isLastStep, onNext }: { isLastStep: boolean; onNext: () => void }) {
  if (!isLastStep) {
    return (
      <button
        type="button"
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="lg:hidden w-full relative bg-background border-t border-card-border">
            ...
            <img 
              src={imageUrl} 
              alt={step.title} 
              className="w-full h-auto aspect-[4/3] object-cover pt-16 drop-shadow-sm"
            />
          </div>
        </div>
      )}
...
function StepGallery({
  ...
  const handleTouchStart = (e: React.TouchEvent) => setNumTouchStart(e.touches[0].clientX);
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 lg:pb-40">
          ...
          <div className="flex flex-col gap-5 mt-4 lg:hidden">
            <TipProBlock />
            <ActionButton isLastStep={intActiveStep === ARR_STEPS.length - 1} onNext={handleNextStep} />
          </div>
        </div>

        {/* Columna Derecha*/}
        <div className="hidden lg:flex lg:col-span-7 flex-col gap-6 sticky top-32 h-fit">
          <StepGallery intActiveStep={intActiveStep} onChangeStep={handleSelectStep} />
          <TipProBlock />
          <ActionButton isLastStep={intActiveStep === ARR_STEPS.length - 1} onNext={handleNextStep} />
        </div>
